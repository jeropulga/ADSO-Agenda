...
import { listarContactos, crearContacto, actualizarContacto, eliminarContactoPorId } from "./api";
...
// Función para cancelar la edición y volver a modo "crear"
  ...
  const contactosFiltrados = contactos.filter((c) => {
    ...
    // MINI RETO 1: También buscamos por teléfono
    ...
    return (
      ...
      telefono.includes(termino) // 🔥 NUEVO: búsqueda por teléfono
    );
  });

  // 2. Ordenamos los contactos filtrados por nombre
  // IMPORTANTE: Usamos [...contactosFiltrados] para crear una copia y NO mutar el estado original
  ...
  // MINI RETO 2: Contador de resultados mostrados
  const cantidadResultados = contactosOrdenados.length;
  const totalContactos = contactos.length;
  const hayFiltroActivo = busqueda.trim() !== "";

  // Texto para el contador con gramática correcta (singular/plural)
  const textoContador = (() => {
    if (hayFiltroActivo) {
      return cantidadResultados === 1
        ? `Mostrando 1 contacto (filtrado)`
        : `Mostrando ${cantidadResultados} contactos (filtrado)`;
    }
    return totalContactos === 1
      ? `Total: 1 contacto`
